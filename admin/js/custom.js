...
jQuery( function( $ ) {
	 ...
	 * Sortable images
	 */
	$('ul.aigwl_gallery_gallery_mtb').sortable({
		items:'li',
	 ...
	 */
	$('.aigwl_gallery_upload_gallery_button').click( function(e){ /* on button click*/
		e.preventDefault();
		...
		var button = $(this),
			...
			/* loop through all the images */
				...
				if( !in_array( attachments[i].id, hiddenfieldvalue ) ) {
					...
					$('ul.aigwl_gallery_gallery_mtb').append('<li data-id='+attachments[i].id+'><span style="background-image:url(' + attachments[i].attributes.url + ')"></span><a href="javascript:void(0);" class="aigwl_gallery_gallery_remove">&times;</a></li>');
			...
			/* refresh sortable */
			$( "ul.aigwl_gallery_gallery_mtb" ).sortable( "refresh" );
	 ...
	 */
	$('body').on('click', '.aigwl_gallery_gallery_remove', function(){
	 ...
	 * Selected item
	 */
	$('body').on('mousedown', 'ul.aigwl_gallery_gallery_mtb li', function(){
		var el = $(this);
		el.parent().find('li').removeClass('aigwl-gallery-active');
		el.addClass('aigwl-gallery');
	});
});